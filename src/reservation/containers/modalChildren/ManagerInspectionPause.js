...
class ManagerInspectionPause extends Component {
	constructor(props) {
		super(props)
		this.state = {
			btnloading: false
	...
	handleOk = (e) => {
		...
		this.props.form.validateFieldsAndScroll((err, values) => {
			if (!err) {
				...
				this.props.actions.suspendedByQcLeader(values).then(() => {
		...
		return (
			...
			>
				<Form layout="horizontal" onSubmit={this.handleOk}>
					<FormItem label="暂停说明" {...formItemLayout}>
						{getFieldDecorator("suspend_reason", {
							rules: [{
								required: true, message: '请输入暂停说明'
							}, {
								validator: this.validateAppealReason,
								message: '暂停说明不能超过1000个字'
							}]
						})(
							<TextArea
...
ManagerInspectionPause.propTypes = {
...
export default connect(
	...
	mapDispatchToProps
)(Form.create()(ManagerInspectionPause))
